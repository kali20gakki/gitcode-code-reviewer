...
在完成上下文构建后，从以下维度审查，并尽量结合项目实际实现给出判断。每个维度下列出常见检查点作为线索，不必逐条机械对照——以是否真正影响行为、可靠性、兼容性、可维护性为准，避免堆砌脱离项目上下文的泛泛意见：

- **目标一致性**
  - 实现是否真的满足 PR 目标，是否只改了表层而漏掉调用链上的必要改动
  - 是否引入与 PR 目标无关的副作用（越界改动），把应拆分的改动混在一起
  - 关联文件、配置项、迁移脚本、文档、调用方是否同步更新

- **正确性与回归风险**
  - 边界条件：空值/null、空集合、零、负数、溢出、超大输入、off-by-one
  - 异常路径与错误传播：异常是否被吞掉、错误码是否正确、失败后状态是否一致
  - 并发与状态切换：竞态、死锁、可见性、不可变假设是否成立
  - 老行为是否被意外改变，回归测试是否覆盖关键分支
  - 类型安全：隐式转换、可选类型、动态类型边界

- **契约兼容性**
  - 函数签名、接口字段、返回值结构是否与上下游一致
  - 序列化格式、错误码、配置项 key、环境变量是否兼容
  - 数据库 schema、迁移是否前向/后向兼容，是否需要灰度
  - 公开 API、协议、事件结构是否破坏现有消费方

- **项目一致性**
  - 是否偏离仓库已有模式（命名、分层、错误处理风格、依赖注入方式）
  - 是否重复造轮子，已有工具函数/基类/组件能否复用
  - 是否引入与项目栈冲突的新依赖或反模式

- **性能与资源**
  - N+1 查询、循环内 I/O、热路径上的 O(n²) 复杂度
  - 不必要的内存分配、大对象拷贝、无界集合或循环
  - 锁竞争、缓存失效、连接池耗尽
  - 资源泄漏：文件句柄、连接、线程、锁未释放
  - 缺失的数据库索引或低效查询

- **安全性**
  - 注入风险：SQL、命令、路径、XSS、模板注入、SSRF
  - 认证与授权：越权访问、垂直/水平权限校验、会话失效
  - 敏感数据：日志/错误信息中的密钥、token、PII 泄露；硬编码凭据
  - 不安全的反序列化、不安全的随机数、弱加密/哈希
  - 输入校验与输出编码是否完整
...
- **可测试性**
  - 关键分支是否有测试支撑；没有测试时，至少指出应补哪类用例（正向、边界、异常）
  - 改动是否可被现有测试框架覆盖，是否需新增 fixture 或 mock
  - 是否存在难以测试的副作用（时间、随机数、外部服务）
